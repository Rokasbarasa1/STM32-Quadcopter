...
double wmm_epoch_year = 0.0;      // Corresponds to: MagneticModel->epoch
...
void parse_wmm_cof_file(MAGtype_MagneticModel *(*magneticmodels)[]) {
    int line_num = 0;
    ...
    (*magneticmodels)[0]->Main_Field_Coeff_H[0] = 0.0;
    (*magneticmodels)[0]->Main_Field_Coeff_G[0] = 0.0;
    (*magneticmodels)[0]->Secular_Var_Coeff_H[0] = 0.0;
    (*magneticmodels)[0]->Secular_Var_Coeff_G[0] = 0.0;
...
void wmm_init(){
    ...
    parse_wmm_cof_file(&MagneticModels);
...
// Cmpute wmm elements given the information about position
...
// Get declination in degreees. Perfect for adding to substracting magnetic yaw
double wmm_get_declination_degrees(){
    if(!wmm_compute_elements_completed) return 0.0;

    return GeoMagneticElements.Decl;
}

// Print out the declination like the wmm code does 
...
// Prevent the usage of the elements after this function is called. To get them again they have to be recalcuated
